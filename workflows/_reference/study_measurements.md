...
Measured 2026-08-05 against `AptnerPass` (`v1_develop` @ `1d0705264b8cb405f63e00820171f2e3e48b0852`),
by reading the `path` and `evidenceRef` strings out of the two `study_guide.json` artifacts that the
C# deep runs produced. The question: where does the `AptnerPass/` prefix come from, and would a
clearer label remove it?

| target | refs | prefixed | resolve as-is | resolve if prefix stripped |
|---|---|---|---|---|
| `guardtec-sse-device-id-string` (run 1, bare label) | 40 | 17 | **0** | **17** |
| `guardtec-sse-d1-v1-compat` (run 2, repo-relative label) | 51 | 20 | **0** | **20** |

**Nothing was misidentified — it was re-rooted.** 37 of 37 prefixed references resolve once
`AptnerPass/` is removed. The repository is `…/AptnerPass/AptnerPass`, so prepending its own
directory name lands one level up, on the repository's PARENT. The authors are not guessing at files;
they are normalizing paths to a root they inferred.

**Three findings kill the two cheap explanations.**

1. **Not copied from the documents.** `device-id-string`'s `spec.md` + `changes.md` contain **zero**
   `AptnerPass/`-prefixed tokens (51 tokens extracted, 0 prefixed), yet its guide carries 17. The
   convention is invented author-side. (`d1-v1-compat` *does* carry 5 prefixed tokens / 10 citations
   in its documents, all of which fail as written and all 5 of which resolve when stripped — a real
   defect in that target's prose, but not the cause here.)
2. **Not fixed by a correct label.** In run 2, after Step 1.3(6) began labelling the prose documents
   `docs/harness/<slug>/spec.md`, three references cite
   `AptnerPass/docs/harness/guardtec-sse-d1-v1-compat/spec.md` and one cites the label unchanged.
   The authors took a correct repo-relative path and prepended to it.
3. **Not a per-author constant, so it cannot be corrected after the fact.** The same file is cited
   both ways inside one run — `LocalCommonSection.cs` 12 prefixed / 11 root-relative,
   `Devicenfo.cs` 10 prefixed / 1 root-relative, `Suprema.cs` 2 prefixed / 6 root-relative. There is
   no majority spelling to normalize to.

**This is what selects option (a).** A 2026-08-05 handoff proposed three remedies — (a) state the
root in the evidence block, (b) add a `repoRoot` field to `args`, (c) declare `cwd ≠ repository` out
of contract — and recommended (c) as cheapest. Finding 1 refutes (c): the prefix is generated by the
authors regardless of what the documents say, and the ledger's earlier framing of it as "cwd-relative
confusion, specific to external-target use" is narrower than the evidence. Finding 2 refutes the
do-nothing option of relying on the label fix. (b) would work but adds a schema field for something
the authors read as prose anyway. Step 1.3(6) now heads the block with the root and forbids
prepending.

**Unverified, and labelled as such.** The fix is a prompt-level instruction; no run has measured it.
The comparable precedent is the label fix, which moved its own class 11 → 0 (§C# Target), but that
