...
// runtime -- gated on verdict.verdict === 'PASS' && A.coldPass === true near the
// bottom of this file. This is not a new pattern: phase 2 ('Evaluate') already skips
...
export const meta = {
  ...
  phases: [
    ...
    { title: 'Cold review', detail: 'runs only when Evaluate returned PASS and coldPass is true, at most once per round' },
  ],
}
...
// change -- the source file contains no backtick and no dollar-brace to escape (AC-8), so
// every section above '## Output Contract' is copied byte-identical from that file.
...
// gated result, not a raw CLI echo. This segment adds only the one condition it alone can
// see -- verdict.verdict === 'PASS' -- plus the defense-in-depth skipL1 check (AC-15).
...
if (verdict.verdict === 'PASS' && A.coldPass === true && !A.skipL1) {
  phase('Cold review')
