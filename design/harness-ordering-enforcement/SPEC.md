...
rev.5가 「나머지 15건은 판정 미완」이라 적은 그 목록은 **커밋된 어느 문서에도 없었다** — 리뷰 워크플로가
`r.verdict.refuted` null 참조로 실패해 `result`가 비었고, 산출물은 세션 디렉터리의 에이전트 트랜스크립트
40개에만 남아 있었다. 18건 전건과 반증 22표를 거기서 복원해 `ADVERSARIAL-c2.md`로 커밋했다(같은 디렉터리).
판정은 분할 후 트리(`develop @ ab515a1`)에 대해 내렸다:

1. **성립 11건**(#1·2·6·8·9·10·11·12·15·16·17) — 전건 rev.5 또는 `3b22b43`가 이미 수정했고, 그 문면이
   분할 후 세 파일 중 예상한 자리에 살아 있다(gate: §Stale Determination·row ④ / harness: §Step 2.6·
   §Architecture Principles #1 / 문서: SPEC·CHANGELOG·ROADMAP). **분할이 만든 회귀 0.**
2. **기각 4건**(#3 malformed 규칙 부재 · #13 AP#1 삭제 미언급 · #14 결정 A 충돌 · #18 I/O 축 뭉뚱그림) —
   두 반증자가 모두 refuted였고 이 판정도 동의한다. 그러나 rev.5가 넷 다 명료화 문면으로 수용했으므로
   되돌리지 않는다: 반증 논거가 옳다는 것과 문면이 틀렸다는 것은 다른 명제다. §3.3a가 그 표본 —
   반증자의 논거(「결정 A는 산출에 관한 것이지 입력이 아니다」)가 곧 §3.3a의 내용이다.
3. **rev.5의 「3건 성립」 문장은 정정한다** — 그 3건은 #5·#7·#4였고, 18건 기준 최종 집계는 성립 14 / 기각 4다.
   「15건」이라는 수 자체는 rev.5가 세 건을 뺀 나머지이므로 맞다.
4. **표결 결측은 복원으로 메워지지 않는다.** 반증자 36 중 22만 판정을 남겨 7건(#5·6·7·10·11·15·16)은 독립
   반증 0회다. 이 rev의 판정은 「수정이 존재하는가 + 당시 재현 기록이 있는가」로만 내렸고, 표결 보충(C안)은
   사용자가 인라인 감사(B안)를 택해 하지 않았다. 콜드 리뷰가 같은 절을 다시 읽는다.
5. **AC-2 후속 실측** — 분할 후 3파일 합 `mtime` 10행 / 11회, 전건 서술, 비교 사이트 0(AC-2 행에 append).
6. **심각도 정정 1건** — #4(AC-2 수치 오류)는 blocking이 아니라 minor: 결론(비교 사이트 0)은 유지됐고
   숫자만 틀렸다.
...
직접 재현·확인**한 것만이다 — 나머지 15건은 판정 미완이며 그렇게 표시한다. **rev.11 정정(2026-09-10):** 그 15건은 이 문서에 열거된 적이 없었다(워크플로 실패로 산출물이 세션 밖에 남지 않음). 복원·판정은 `ADVERSARIAL-c2.md`; 결과 성립 11 / 기각 4.
...
| AC-2 | C2 적용 후 `skills/harness/SKILL.md`에 mtime **비교**를 수행하는 사이트가 0건 | `grep -n mtime` → 9행 / 10회, 전건 서술 (**충족**) — **rev.11 분할 후 재측정**: 3파일 합 10행 / 11회(harness 3/3, gate 5/5, build 2/3), 전건 서술, 비교 사이트 0. 명령은 `ADVERSARIAL-c2.md` §6 |
