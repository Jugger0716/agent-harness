...
`templates/_shared/session_conflict.md` §Gate Procedure (`{current_skill}` = `test-gen`). Cited by
...
Fires when `.harness/state.json` exists AND (`skill` is absent OR `skill != "test-gen"`) — another
