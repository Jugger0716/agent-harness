...
- **Graceful fallback:** if a segment errors (launch failure, script error, schema-invalid result), print `[test-gen] ⚠ Workflow engine unavailable — falling back to the inline single path.` (in `user_lang`), set `mode → single`, `path_resolved → inline`, and continue inline. Never error out. **A permission denial is NOT one of those errors** — route it to the denial branch of `templates/_shared/mode_gate.md` rule 3 (single source: disclose it, never downgrade silently).
...
If `.harness/state.json` exists AND (`skill` is absent OR `skill != "test-gen"`), a different
...
Ask via AskUserQuestion (in `user_lang`):
  ...
  question: "A `/{skill|'unknown'}` session exists in this directory (task: `{task}`, phase: `{phase}`, docs: `{docs_path}`). Starting /test-gen here will delete it. Delete it and start /test-gen?"
  options:
    - label: "Delete and start" / description: "Delete .harness/ and proceed with /test-gen"
...
§Gate Procedure (`{current_skill}` = `test-gen`), cited by name and not restated here.
