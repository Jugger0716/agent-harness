...
Status block shape + label rules: see `templates/_shared/status_format.md`. Ship uses the `[harness:ship]` prefix and its own ship-specific block shown below.
...
- `version_bump`:
  ...
  - `substep == "version_bump_pass2_restore_failed"` → do NOT auto-resume into Pass 2 or any later stage. Halt with a clear error naming the affected file(s) and instructing the user to inspect and manually restore them before restarting the stage (same "surface, don't auto-recover" precedent as the state-consistency check below)
- `changelog` → Step 3
- `build_verify` → Step 4
- `code_review` → Step 5
- `git_ops`:
...
```json
{
  ...
  "stage_results": {
    "version_bump_files": [],
    "merge_to_base": {                  // initialized at Setup; populated by Stage 6.5
      ...
      "push_retry_count": 0             // persisted retry counter for Stage 6.5 step 7
    }
  },
...
**substep** tracks intra-stage progress for recovery. Valid values (exhaustive enum, grouped by stage):
...
- `"version_bump_pass2_restore_failed"` — Pass 2's post-write byte assertion failed AND the automatic restore of the files it had written also failed; manual file inspection is required before this stage can resume (see Pass 2 step 5)
...
1. Detect the original line-ending convention by inspecting raw bytes (e.g., `b"\r\n" in raw`), the UTF-8 BOM prefix (`b"\xef\xbb\xbf"` at file start), and the original trailing-newline state, and **keep the full original raw bytes in memory**. This is critical because `.claude-plugin/*.json` files may use CRLF on Windows-authored repos and may have a BOM. These three detected properties and the raw-byte copy are no longer inputs to a re-serialization step (see step 4) — they now serve as (a) the restore source if the step 5 post-write assertion fails, and (b) the reference the step 5 byte assertion diffs against.
2. Parse the file as JSON (UTF-8) as a **read-only oracle, not a mutation path** — it exists only to confirm the file is valid JSON and to capture the current value at each Pass-1-identified key path (used by step 3's branching and by step 5's post-write value check); the parsed object itself is never re-serialized back to disk. On parse failure, do NOT fall through to string replace — instead emit a warning (`⚠ Invalid JSON at <path> — cannot determine version key path`) and ask the user via AskUserQuestion (`user_lang`) to "skip this file" or "abort stage". On "skip", exclude the file from Pass 2 and proceed with the remaining files; on "abort", halt the stage (state.json `substep` retains its current value for resume).
...
4. **Anchor by key path and edit that one line in place — never a bare `"version":` first-match.** For every key path from step 3 that requires a real replacement (the branch where the current value equals `{current_version}`), locate that key path's own line by narrowing on structure, not on a bare string search: `marketplace.json` is a concrete counter-example where a first-match search breaks, because `$.metadata.version` and `$.plugins[i].version` can hold the identical literal version string at different indentation levels, so `"version":` alone cannot tell them apart. Narrow first to the enclosing object's line span (the specific `plugins[i]` object's bounds, or the top-level `metadata` object's bounds), then within that span find the single `"version": "<value>"` line. If more than one match remains even after narrowing to the object's line span, do not guess — route to the same skip/abort AskUserQuestion used in step 2. Once the line is uniquely identified, replace only the value token on that line (the quoted version string); every other character on that line, and every other line in the file, stays untouched. This is a targeted single-line in-place edit, not a parse-mutate-serialize round trip — key order, indentation, the line-ending convention (CRLF vs LF), the BOM prefix, the trailing-newline state, and the one-line `keywords` array are all preserved automatically, because nothing outside the identified line is ever rewritten.
5. Write the file back, then **re-parse it as a second read-only oracle**: confirm JSON validity and that the new version string is present at the same key path (as before). In addition, run the **byte assertion**: every line except **each replaced key path's own target line — the line step 4 identified BEFORE the write, one per key path it actually replaced, never inferred afterwards from whatever turned out to have changed** — must be byte-identical to the raw bytes captured in step 1, and the total line count must be unchanged. **The two checks' jobs are distinct, and neither substitutes for the other**: a wrong-line edit (e.g., `description` mutated instead of `version`) is caught by the post-write key-path value re-check above, NOT by this assertion — a line step 4 wrongly targeted is excluded from the assertion by construction, which is precisely why the value re-check (the new version string must be present AT the key path) is the check that fails in that case. The byte assertion's own job is everything outside those target lines: collateral corruption of untargeted lines, and an accidental full re-serialization (a 3-line version bump silently producing a much larger diff). **If the post-write key-path value re-check fails** (the new version string is NOT present at a key path step 4 actually replaced — the wrong-line-edit case this step names above, and the only check that detects it): treat it exactly as a byte-assertion failure — restore, abort Pass 2, block Stage 3 (`changelog`) onward, and report, per the branch immediately below. The check's scope is step 4's actual replacements only; the key-absent, idempotent-skip and drift-warning branches of step 3 are not replacements and are not re-checked here. Without this branch the check this step designates as the sole detector of a wrong-line edit would have no defined outcome, and Stage 6a's commit gate cannot cover for it: that gate shows `git diff --cached --stat`, which renders a one-line wrong edit and a one-line correct edit identically. **If the byte assertion fails**: restore every file this Pass 2 run has written so far (this file and any earlier ones in the same run, whether written via the JSON-aware path above (its numbered step 1) or the plain string-replace path below (its first bullet) — both keep a raw-byte backup in memory before writing) from those in-memory backups, abort Pass 2, and halt the entire `/ship` session — do NOT proceed to Stage 3 (`changelog`) or any later stage, mirroring the restore-failure branch immediately below — then report the failure and the restored file list to the user; the user must inspect the reported files and restart `/ship` from `version_bump` manually. **If the restore itself fails**, set `substep → "version_bump_pass2_restore_failed"` and halt the entire `/ship` session rather than leaving `substep` at a value that lets Session Recovery silently re-enter Pass 2 over a partially-mutated file (see the Session Recovery entry for this substep).

**Operator self-check**: a 3-line version bump should produce a 3-line diff — one changed line per key path actually replaced. Anything larger means step 4's line-anchored edit did not hold and some form of re-serialization happened.

**Important regression-blocker**: Do NOT apply naive string replace to `.claude-plugin/*.json` files. JSON path matching ensures that other fields (e.g., `description: "Initial 8.2.0 release notes…"`) containing the same version string by coincidence are NOT modified. The key-path-anchored, line-scoped in-place edit in step 4 above is not a naive string replace either — it locates one specific line via the parsed key path's object boundaries before touching anything, the same protection this paragraph has always required.

**For all other detected files** (standard package manifests, source constants):

- Read the file and **keep the full original raw bytes in memory before writing** (same role as the JSON-aware path's step 1 — the restore source if a later file's step-5 byte assertion aborts this Pass 2 run).
- Replace `{current_version}` with `{release_version}` (string replace).
- Write the file back.
...
> Bug-fix audit IDs for this stage (formerly inline) now live in CHANGELOG.md.
...
**Halt behavior**: `state.json` is preserved as-is — `substep` retains its current value (typically `"git_branch_pushed"` since the entry guard runs before any 6.5-internal substep transition; could be `"merge_base_pending"` or `"merge_base_done"` if the session is being resumed mid-6.5). When the user later switches back to `{state.branch}` and re-invokes /ship, the jump table re-routes to Stage 6.5 entry (or to step 2 / step 5 if a 6.5-internal substep is active) and **this entry guard is re-executed**, so the branch-equality check is re-evaluated; if the user is now on the correct branch the guard passes and execution continues. The guard is therefore safe to re-run any number of times.
...
```
- **Create PR** → execute `gh pr create --base {base_branch} --head {release_branch} --title "Release {release_version}"`. On success: parse the printed PR URL, store `state.stage_results.merge_to_base.path_a_pr_url = "<url>"`, set `substep → "merge_base_pushed"` (treating PR creation as the "push" equivalent — the tag still pushes to release_branch lineage at 6c-ii), execute `git checkout {release_branch}` to guarantee HEAD context for 6c-ii (halt on checkout failure), continue to 6c-ii. On failure: if stderr indicates "a pull request for branch ... already exists" (race condition where a PR was created between A.1's check and now) → re-run A.1's `gh pr list` to fetch the URL and proceed via the "existing PR" branch above; otherwise → re-display this gate (the user can choose Skip or Stop to escape).
- **Skip Stage 6.5** → `substep → "merge_base_pushed"`, run `git checkout {release_branch}` (defensive — Path A has not changed branches but ensures HEAD context for 6c-ii regardless of any prior state; halt on failure), warn user that base_branch will lag this release, continue to 6c-ii.
...
3. **Symlink escape prevention**: **Always** verify `Path('.harness').resolve()` ⊆ `Path.cwd().resolve()` (no `has_git` condition, no skip path; Item 2 already implies this, but Item 3 stays for explicit escape-rejection symmetry). If the resolved real path escapes the working directory → **ABORT**, warn user: `[harness:ship] Safety Guard: .harness/ resolves outside cwd — refusing to delete.`
