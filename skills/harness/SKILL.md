...
**doctor carve-out — evaluate this FIRST, before item 1 below.**
...
/harness doctor                          → read-only diagnostic; the bypass contract lives in `## Sub-command: doctor`, not here
```
...
rule, with the prefix `[harness doctor]`: labels English raw, values and remediation in
`user_lang`, one `✓` or `⚠` per item. doctor does not read `.harness/state.json` and does not
render that file's session status block.
...
present and the count is zero → `✓`.
Remediation differs by location, and which one applies is decided at run time, never asserted in
advance. The marketplace location is a git clone here, but a clone is only restorable when
`git rev-parse --is-inside-work-tree` prints exactly `true` for it; where that holds, the work-tree
repair `.gitattributes` prescribes applies, and where it does not, a fresh install of the plugin is
the only remedy. A cache version directory is judged by the same run-time test rather than by an
assumption about caches. That repair removes the files and checks them out again — it is this
pair, not a bare `git checkout`:
...
contradict the rule being reported.
...
directory. This is the same rule as `{CLAUDE_PLUGIN_ROOT}/skills/team-memory/SKILL.md` §Step 1.5
item 1, cited by name rather than restated.

④ **Version and content drift**, on three axes, over the known install locations above.
   (a) Directory name against the `plugin.json` inside it. The source of truth is that
   `plugin.json` `$.version`, not the directory name. A directory whose name carries a backup
   suffix is excluded from this axis and listed separately. Print the resolved value of
   ...
   (c) Content identity. The left-hand side is `git ls-files`. The right-hand side is the
   ...
   **Choosing the comparison copy**, in order: ① if `${CLAUDE_PLUGIN_ROOT}` resolved, compare that
   copy and mark it `(loaded copy)` — it is the one actually running; ② otherwise take the known
   install locations above — the marketplace copy, and those cache version directories whose
   ...
   picking one. ③ If the path from ① is none of the paths from ②, print `⚠` — the copy compared is
   not the copy running.
...
`{CLAUDE_PLUGIN_ROOT}/skills/team-memory/SKILL.md` §Step 1.5 items 1–2, together with the §Step 1
Storage Criteria table that item 2 itself names, by name — do not restate their bodies here,
neither the slug prefixes nor the category directory names. (ii) That plugin-root path is the read
...
sources in its own precedence order, treat the winning source as winning wholesale, and honour its
column-zero parsing rule — an indented or fenced occurrence is an example, not a declaration.
Report which source won and what the final resolved reading is.
