...
`templates/_shared/session_conflict.md` §Gate Procedure (`{current_skill}` = `migrate`). Cited by
...
Fires when `.harness/state.json` exists AND (`skill` is absent OR `skill != "migrate"`) — another
