...
description: Transform an AI-assisted development artifact (a /harness output directory, a whole project, or a git diff) into a 7-section, 3-tier study guide (concept explanation, real code excerpts, interview Q&A, hands-on exercises, engineering principles, anti-patterns, glossary) so a developer can extract the transferable technical knowledge and engineering judgment the finished work demonstrates — verified revision material, not a usage guide and not a record of why this artifact was built. Machine-checked provenance (repo-quote re-verification against the actual files, inference-vs-repo claim basis) and a self-contained static HTML report accompany every guide.
---
...
- **Graceful fallback.** If the Author segment errors (launch failure, script error, schema-invalid result, or all lenses/all buckets failed): print `[harness] ⚠ Workflow engine unavailable — falling back to the inline quick path.` (in `user_lang`), set `mode → quick`, `path_resolved → inline`, and re-run authoring inline for the ALREADY-APPROVED topic list, capped to the first 5 if it exceeds quick's range. Print 1 line naming this as a scope reduction from what the user approved (never present a shrunk guide as if it were the full approved one). **A permission denial is NOT one of those errors** — route it to the denial branch of `templates/_shared/mode_gate.md` rule 3 (single source: disclose it, never downgrade silently).
...
3. **Gather target evidence** (orchestrator-inline, cheap — this is NOT the Author segment). The branches below are keyed on the **resolved target kind**, not on which flag was typed — an auto-detected run (no flag at all, step 2) resolves a harness slug and therefore takes the first branch:
   - **harness slug target** (explicit `--harness <slug>`, or a slug resolved by auto-detect): read `spec.md` + `changes.md` in full, **then follow them into the code they name.** Those two documents are a decision ledger; stopping at them is precisely what produces a guide about prose instead of about code. Sub-procedure:
     ...
     2. **Extract candidate paths.** From both documents — inside code spans/fences and in running text — collect repo-relative tokens matching `[\w./-]+\.(csproj|json|yaml|html|toml|java|tsx|cpp|mjs|cjs|yml|ps1|sql|css|js|ts|py|go|rs|rb|cs|sh|cc|c|h)\b` and count occurrences of each. **The alternation is ordered longest-extension-first and closed with `\b` — do not "tidy" it into alphabetical order, and do not "harden" the ending into `(?![\w])`.** Ripgrep is leftmost-*first*, not leftmost-longest, so an alphabetical alternation with **no trailing anchor** truncates `state.json` to `state.js`, `html_shell.html` to `html_shell.h` and `GateService.cs` to `GateService.c` — paths that do not exist, deleted silently by step 3, taking the config bucket with them. **The trailing `\b` is what holds that off** (measured 2026-08-05: with it present the same alphabetical alternation truncates nothing), which makes the SECOND prohibition the load-bearing one: `(?![\w])` is semantically identical but ripgrep's default engine has no look-around and dies with `regex parse error` in the exact tool step 1 tells you to use — and an editor who clears that error by DELETING the anchor instead of restoring `\b` re-arms every truncation at once. Keep both anyway: length ordering is free insurance that does not depend on the anchor surviving the next edit. Both failure modes are measured (`workflows/_reference/study_measurements.md` §Outline Patterns). **Adding an extension is allowed and is NOT what those two prohibitions forbid** — but insert it at its LENGTH position, never at the end, and re-measure anything that depended on the old set: `csproj`/`cs` were added 2026-08-05 after a C# target extracted **zero** paths under the previous set, and a language admitted here without an outline row below is a language step 5 cannot window (§Outline Patterns). Known gap, accepted: extension-less files such as `.gitattributes` or `Dockerfile` cannot be found by an extension pattern, so they never enter evidence this way even though §3.4a can classify one as `config` if a topic quotes it from elsewhere.
     ...
     3. **Exclude, then order deterministically.** Drop prose (`.md`/`.mdx`/`.txt`/`.rst`/`.adoc`), **everything under `docs/`** (this skill's own output — otherwise a previous round's report qualifies as "code"), non-existent paths, binaries, **empty files (0 bytes)**, and anything failing `resolve ⊆ cwd`. Order by citation count descending, then **path ascending** as the tie-break.
        ...
        **Test `resolve ⊆ cwd` on `..` segments — do NOT reach it by resolving the token first.** That rule is an escape-attempt guard, and an elision token (`.../CommonSection.cs`) is not an escape attempt; it is the exact shape the recovery paragraph above exists for. Resolving the token to decide the question answers a different one: measured, `Path(".../CommonSection.cs").resolve()` leaves the repository on Windows, so 8 elision tokens across two targets were classified as escapes and dropped **before** recovery could run, understating one target's distinct-token resolution by 29 points (21% → 50%). Order the drops so that non-existence is decided — and recovery attempted — before any escape verdict (`workflows/_reference/study_measurements.md` §Citation Resolution).

        **Record what dropped, and carry the tally to the §1.6 gate.** Count the extracted tokens `N`, the tokens that reach an existing admissible file `R` (recovery included), and — of the rest — the ones dropped as **non-existent**, `D`. Keep `D` separate from the `docs/`, binary, empty-file and escape drops: those are exclusions by design, whereas `D` is the only class that means *the documents describe files this checkout does not have*. Step 1.6 discloses it, and nothing else in this skill does.
     4. **Budget — the WORKFLOW path gets LESS than quick, not more.** deep/thorough = **550 lines** total / **300 lines per file** / at most **12 files**; quick = **600 lines** total / **300 lines per file** / at most **12 files** (quick holds this evidence in ONE context while authoring 3-5 topics and writing a ~60KB report — see §Risks).
        ...
        **On the inline path filling is free; on the WORKFLOW path it is not, and the line budget is not what binds there.** quick holds the evidence in one context with no serialization cap, so it simply fills. deep/thorough serialize through `args.sharedEvidence` under a hard 35,000-character cap, and characters-per-line is language-dependent — measured, a filled 550-line gather runs from 24,923 to **31,945** characters with the language, and at the high end the code side alone eats the whole allowance (`workflows/_reference/study_measurements.md` §Serialization Cost). So on the workflow path, **Step 2-W reserves prose FIRST and the reserved remainder sizes this step's fill**: that reservation is a fixed **8,750-character** prose budget split between the two documents (Step 2-W), so this step fills against `35,000 − <reserved> − the block's own overhead`. **Compute the reservation BEFORE starting this fill.** It depends only on the two documents' lengths — never on what this step selects — so there is no circularity, and it is what makes the §1.6 gate able to state the reduction before the spend rather than after it. Fill until the line budget OR that character remainder runs out, whichever binds first.
        ...
        The inversion is counterintuitive enough to state outright. quick holds the evidence in one context and never serializes it; deep/thorough must pass it through `args.sharedEvidence`, which the orchestrator re-emits as tool-call output and the segment then re-embeds in EVERY lens and bucket prompt — 6 agents in deep, 7+ in thorough. **Evidence cost is multiplied by the agent count on the workflow path and paid once on the inline one**, so the budgets run opposite to the modes' apparent size. **What deep and thorough buy is more ANALYSIS over the same evidence — 8-15 topics, three independent lenses, a critic and an assemble pass — never more SERIALIZED evidence.** Read that last word literally: the cap bounds what this step *pays to transmit*, not what the sub-agents can *see*. They keep their own file tools, and measured 2026-08-05 they used them — one authored topic quoted the real body of a file that appeared nowhere in `sharedEvidence`, correctly and with a verifying line range. That is a better outcome than inventing the quote, and it is the reason the badge is computed from a re-read (§3.4) rather than from what was sent. But it means the cap is a cost control, not an evidence boundary, and any sentence here that reads as "the authors see exactly this and no more" is false. (Measured: the old, larger budget row serialized to ~56,000 characters, which the orchestrator cannot re-emit in one dispatch call at all, and hand-transcribing it would corrupt the §3.4 quote re-verification the whole guide rests on — the exact failure mode §3.2.2 forbids for the render. `workflows/_reference/study_measurements.md` §Serialization Cost.)
     ...
     5. **Over the per-file cap, never truncate from the head.** Grep the top-level declaration outline **using the pattern set for the file's extension (table below)**, with line numbers, then read **at most two 150-line windows** centred on declarations the documents named by name.
        ...
        **Read `\|` in the table below as a plain `|`.** A pipe inside a GFM table cell has to be written `\|` or it ends the cell, so the alternations appear escaped — but `\|` in a regex means a LITERAL pipe character, which turns `(class\|interface)` into a pattern matching the text `class|interface` and silently matches nothing. **Un-escape every `\|` to `|` before handing a pattern to Grep.** Measured, the `.java` type pattern copied WITH its table escaping returns **0 matches and no parse error** on a file where the un-escaped form finds a `public class` declaration — ripgrep accepts it and silently finds nothing, which is worse than being rejected (`workflows/_reference/study_measurements.md` §Outline Patterns). Same failure family as step 2's `(?![\w])`: a pattern that reads correctly in the document and dies in the tool this step tells you to use.
        ...
        | `.cs` | `^\s*(public \|internal \|protected \|private )?(abstract \|sealed \|static \|partial )*(class\|interface\|enum\|struct\|record) ([A-Za-z_]\w*)` / `^\s+(public\|internal\|protected\|private)[^;={]*\s([A-Za-z_]\w*)\(` |
        ...
        **The `.cs` row anchors its type pattern with `^\s*`, not `^`, and that difference is measured.** C# block-scoped namespaces indent every type declaration, so the column-0 anchor the `.java` row uses finds **0** types in all six real C# files tested — the identical silent death this table was created to end, one language later. Port a row between languages and you inherit its anchoring assumption, not its results: re-measure before adding one (`workflows/_reference/study_measurements.md` §Outline Patterns).
     ...
     6. Append to the gathered evidence as a `## Cited Source Files` section, each file headed `path (lines a-b of T, cited N times)`. **Label the two prose documents with their repo-relative paths as well — `docs/harness/<slug>/spec.md`, never a bare `spec.md`.** Whatever heading this step writes is what the authors copy into `evidenceRef`, and §3.5 resolves that string against the repository before it will render a `repo` badge: measured 2026-08-05 on a C# target, a bare-filename label sent **11 of 29** repo-basis claims to auto-downgraded `inference`, because `spec.md` does not exist at the repository root. The label is not presentation — it is the citation the guide ends up carrying.

        **Head the block with the analyzed repository's root, and forbid re-rooting.** Emit one line before the first file heading (in English, like the rest of the block's machine-read chrome): `All paths below are relative to the analyzed repository root: <absolute path>. Reproduce them exactly in `path`/`evidenceRef` — never prepend a directory.` It costs about 80 characters and counts toward the block's own overhead like every other header line (Step 2-W). **Correct labels alone are measurably NOT enough**: on an external target the authors invented a root of their own and prefixed it onto paths that were already right — **every** prefixed reference across two runs (17 of 17, then 20 of 20) resolves the moment the prefix is stripped, so nothing was misidentified, only re-rooted one level onto the repository's PARENT. It is not copied from the documents either: the prefix appears in force on a target whose two documents contain **zero** prefixed paths. And in the post-label-fix run, three references prefixed the very `docs/harness/<slug>/spec.md` label this step had just made repo-relative, while a fourth used it unchanged — same file, both spellings, one run. **This is why the fix is a stated root and not a clearer label** (`workflows/_reference/study_measurements.md` §Path Root).

        The residual is stated rather than hidden: this is a prompt-level instruction and therefore unverified until a run measures it, exactly like the label fix before it — which did move its own failure class 11 → 0. What makes it checkable is that §3.5 already resolves `evidenceRef` against the repository and downgrades what does not exist, so a re-rooted path still fails safely; the line changes how often that happens, never whether the badge tells the truth.
   ...
   - `--diff <range>`: run `git diff --stat <range>` (cheap, shown at the gate) plus a **capped** unified diff (first ~3000 lines / ~150 files) for the deeper evidence passed to lenses; if capped, note the truncation once (in `user_lang`) so the user knows later topics may have thinner evidence. **No `--` before the range.** `git diff --stat -- <range>` puts the range after the pathspec separator, so git reads it as a PATH, matches nothing, and exits **0 with empty output** — measured on a 10-file range that the correct form renders in 11 lines. The separator exists to disambiguate PATHS, and this call passes none; the failure is silent and lands on the one piece of evidence the §1.6 gate shows the user, which would then show nothing while the run continued. (Step 1.2's rule still holds: pass `range` as an argv array element, never through a shell string.)
...
6. **Combined confirmation gate** (single gate — cost, topics, and Artifact disclosure together, so the user is not asked twice before the one irreversible-spend point):
   ...
   - **one citation-resolution line on a harness-slug target** — `Cited paths : <R> of <N> resolved (<P>%) — <D> not found in this checkout` — read straight off the tally Step 1.3(3) recorded. **`--diff` and `--project` omit this line entirely, and that is not an oversight**: the tally is produced by Step 1.3(3), which lives in the harness-slug sub-procedure, and neither other branch extracts cited paths at all — a diff or a structural pass IS the evidence rather than a set of references to it, so there is no denominator to report and no checkout it could disagree with. (Found by running `--diff` against an earlier revision of this bullet that said "unconditionally"; a line whose inputs do not exist on two of three branches is a contract that cannot be met.) The evidence line above reports what the gather KEPT; this one reports what the documents asked for and did not get, which is a different quantity and the only one that can expose a **checkout mismatch**. That failure is silent everywhere else: when the artifact describes work on a branch this tree is not on, the missing files are dropped correctly, and §3.4's quote re-verification, §3.5's path resolution and §3.9's excerpt counts then all pass **green** against whatever files DO exist — so the guide teaches the pre-change code wearing a verified badge. Measured, the documented mismatch case resolved 14 of 43 paths and the guide it produced was green throughout (`workflows/_reference/study_measurements.md` §Citation Resolution).
   - **Do NOT promote that line into a threshold warning, and do not add a cut-off.** Measured across 16 targets on one repository, `R/N` runs from **20% to 74%** in a continuous band with no gap, and it is low for reasons that have nothing to do with a mismatch — a spec names files its work will *create* (Step 1.3(2)'s second known gap), abbreviates deep paths past what recovery can accept, and spells the same file two ways. The documented-mismatch target sits at **33%**, three targets score at or below it with their checkout status never established either way, and the four targets outside that epic span 20–40% — so a cut-off that catches the real case cannot separate it from the ordinary ones, and would fire on the majority of sound targets. A number the user reads beside the topic list is worth more than a badge that is always lit (`workflows/_reference/study_measurements.md` §Citation Resolution).
   ...
   Workflow {
     ...
     args: {
       ...
       sharedEvidence: <content gathered in Step 1.3, capped per the rule below>,
         ...
         // truncation notice: `<file> : kept <K> of <T> characters (through line <L>)`, where
         // `<file>` is the REPO-RELATIVE PATH for the same reason Step 1.3(6) requires it of the
         // evidence label — a bare filename here is the one the authors copy into `evidenceRef`. **This
...
6. **Escaping (HTML output; in this exact order — order is load-bearing, reversing it double-escapes `&`):** `&` → `&amp;`, then `<` → `&lt;`, then `>` → `&gt;`. Content is spliced in as escaped semantic HTML (never a JSON data-island — an excerpt containing `</script>` would otherwise truncate the page).
   ...
   **What must be escaped — EVERY model-authored string that reaches the page, with no exceptions:** topic `title`, `tier`, `concept`, every excerpt's `code` and `explanation`, every Q&A `question`/`answer`/`difficulty`, the exercise's `prompt`/`hint`/`answer`/`difficulty`, every decision/anti-pattern/glossary free-text field and `evidenceRef`, every `furtherReading` `url`+`note`, the related-topic titles, **and the text substituted into `__STUDY_TITLE__` / `__STUDY_GENERATED__` / `__STUDY_DISCLOSURE__`**. Escape a string once, at insertion; never twice. Only the shell's own literal markup and the class/attribute names from the §3.3 table are written unescaped — those are ours, not the model's. (A field omitted from this list is a live markup-injection hole: `title` alone goes straight into `<h2>`, and this skill's subject matter is code, so `<` in a title is likely, not exotic.)
   ...
   - **`<` vs `&lt;` cross-check:** before escaping, sum `count('<')` across **exactly the same string set enumerated above** (if the two lists ever diverge, this check produces false alarms — keep them in sync) = `pre_count`. After the full render, count literal `&lt;` occurrences in the written file = `post_count`. `pre_count == post_count` must hold (the shell's own literal tags are never converted, so they do not pollute the count). Mismatch → escaping failed somewhere — warn and do not publish. **This check counts `<` and is therefore blind to a missing `&` escape**, including in the rendered heading LABELS this section names as escape targets: measured 2026-08-05, an unescaped `&` in `Interview Q&A`'s localized label passed the cross-check untouched and was caught only by §3.8's non-empty section-key comparison, which matches on the rendered heading text. Do not read §3.8 as a redundant formality — under this one it is the only net.
