...
6. **Combined confirmation gate** (single gate — cost, topics, and Artifact disclosure together, so the user is not asked twice before the one irreversible-spend point):
   ...
   - **one citation-resolution line, unconditionally** — `Cited paths : <R> of <N> resolved (<P>%) — <D> not found in this checkout` — read straight off the tally Step 1.3(3) recorded. The evidence line above reports what the gather KEPT; this one reports what the documents asked for and did not get, which is a different quantity and the only one that can expose a **checkout mismatch**. That failure is silent everywhere else: when the artifact describes work on a branch this tree is not on, the missing files are dropped correctly, and §3.4's quote re-verification, §3.5's path resolution and §3.9's excerpt counts then all pass **green** against whatever files DO exist — so the guide teaches the pre-change code wearing a verified badge. Measured, the documented mismatch case resolved 14 of 43 paths and the guide it produced was green throughout (`workflows/_reference/study_measurements.md` §Citation Resolution).
