...
1. **(a) Concept explanation** — plain-language explanation of the topic. **≤200 words.** This is the single biggest length-cap risk in the whole guide — stop at 200 words even if more could be said; depth belongs in (e)/(f), not a longer (a).
...
5. **(e) Design decisions** — the rationale behind how this topic was built, including rejected alternatives/trade-offs where the evidence has them. Each item needs `basis`: `'repo'` (evidence explicitly states this reasoning — then `evidenceRef` is REQUIRED, a real path the reasoning came from) or `'inference'` (your own reasonable inference — say so, do not present it as if the repo said it).
...
- `concept`: ≤200 words, render in {user_lang}
...
- `decisions`: `{ decision, rationale, rejectedAlternatives[], basis, evidenceRef? }` items — `rejectedAlternatives` is a **list of strings** (one rejected option per entry, each naming the option and why it lost). The workflow-path schema declares the same array type, so the object shape is identical whichever path authored it.
