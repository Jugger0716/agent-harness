...
- **Interactive pickers hold max 4 options** (AskUserQuestion limit) — `frontier` replaces `all-opus` in the picker. `all-opus` stays fully supported via `--model-config all-opus` or "Other: executor:opus,advisor:opus,evaluator:opus".
