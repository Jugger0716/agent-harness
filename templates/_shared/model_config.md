...
- Set **once** at session start (stored in `state.json.model_config`); not changed mid-session (sole exception: the model fallback chain below may downgrade a cell mid-session on a sunset model id).
...
- **Applying:** when launching a sub-agent, if `model_config.preset != "default"`, pass `model` per the role's preset cell. Sub-agents never read `state.json` for model config — the orchestrator passes `model` at launch. Ad-hoc (non-template) dispatches follow `templates/_shared/adhoc_dispatch.md` §Model Routing, which takes precedence for them.
